...
| ---------------------------------------- | ---------------------------------------- | ---------------------------------------- |
| **drop table **表名称                       | **truncate table **表名称                   | **delete from** 表名称 <br>**where** 列名称 = 值 |
| 删除内容和定义，释放空间。<br>简单来说就是**把整个表去掉**.以后要新增数据是不可能的,除非新增一个表。 | 清空表中的数据;<br> 删除内容、释放空间但不删除定义(**保留表的数据结构**)。<br> 与drop不同的是,只是清空表数据而已。 | 删除表中的数据;<br> delete 语句用于**删除表中的行**。      |
| drop语句将删除表的结构被依赖的约束（constrain),触发器（trigger)索引（index);<br>依赖于该表的存储过程/函数将被保留，但其状态会变为：invalid。 | truncate 不能删除行数据,要删就要把表清空。               | delete语句执行删除的过程是每次从表中删除一行，并且同时将该行的删除操作作为事务记录在日志中保存,  以便进行进行回滚操作。 |
